...
/* Builds repository.json + SHA256SUMS for this repository.
 *
 * Signing: set MODULE_REPOSITORY_SIGNING_JWK (private P-256 JWK) to publish a
 * trusted, signed index. Without it the script still produces a valid index
 * with unsigned (empty) signature fields: Synthetiq Player accepts such an
 * index as a *community repository*, so the modules stay installable, while
 * an index signed with the testing key is treated as trusted. */
...
const privateKey = privateJwkRaw
  ? crypto.createPrivateKey({ key: JSON.parse(privateJwkRaw), format: 'jwk' })
  : null;
if (!privateKey) {
  console.warn('MODULE_REPOSITORY_SIGNING_JWK is not set: writing an unsigned (community) index.');
}
...
const releaseBase = 'https://github.com/kas021/Testing-Modules-1-2-3-4/releases/download';
const releaseRepoPath = '/kas021/Testing-Modules-1-2-3-4/releases/download';
...
function packagePath(tag, file) {
  return `${releaseRepoPath}/${tag}/${encodeURIComponent(path.basename(file))}`;
}
...
console.log(`Validated ${modules.length} modules for Bundle ${catalogue.bundleVersion}` +
  (privateKey ? ' and signed the index.' : ' (unsigned community index).'));
